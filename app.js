(() => {
  ...
  function autoMixStep(){
    const a = state.decks.A.video, b = state.decks.B.video;
    if(a.duration && a.duration - a.currentTime < 10 && state.tracks.length > 1){
      if(b.paused || !b.src) loadTrackToDeck((currentTrackIndex('A')+1)%state.tracks.length, 'B', true);
      const p = Math.min(1, (10 - (a.duration - a.currentTime)) / 10); crossfader.value = Math.round(p*100); updateAudioMix();
    }
    if(b.duration && b.duration - b.currentTime < 10 && state.tracks.length > 1){
      if(a.paused || !a.src) loadTrackToDeck((currentTrackIndex('B')+1)%state.tracks.length, 'A', true);
      const p = Math.min(1, (10 - (b.duration - b.currentTime)) / 10); crossfader.value = Math.round((1-p)*100); updateAudioMix();
    }
  }

  function currentTrackIndex(deck){ return state.tracks.findIndex(t => t.url === state.decks[deck].video.src); }
  ...
  autoMixBtn.onclick = () => { state.autoMix = !state.autoMix; autoMixBtn.classList.toggle('selected', state.autoMix); log(state.autoMix ? 'Auto Mix activado.' : 'Auto Mix desactivado.'); };
